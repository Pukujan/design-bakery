 ...
 *
 * The two assertions worth protecting above all others:
 *
 ...
 *    key-for-key against a literal list. Adding an item-level or gold-label
 *    field to a chart row has to fail here first, where a reviewer will see it.
 */
...
/** A dataset built here, so the edge cases do not depend on eval-lab's numbers. */
function syntheticDataset(overrides = {}) {
  const entity = (id, options) => ({
    ...
    modelFamily: options.family ?? 'Test',
    modelId: `test/${id}`,
    paramsB: null,
    route: `route/${id}`,
    settings: {
  ...
  const observation = (id, slice, sliceValue, value, n = 10, ci = 0.08) => ({
    entity: id,
    slice,
    sliceValue,
    metric: 'acc',
  ...
  for (const entry of entities) {
    const base = { a: 0.9, b: 0.7, c: 0.5, d: 0.3 }[entry.id];
    observations.push(observation(entry.id, 'overall', 'all', base, 10));
    observations.push(observation(entry.id, 'mode', 'pairwise', base + 0.05, 6));
    // `d` publishes no `single` slice: it must be counted as missing, never filled in.
    if (entry.id !== 'd') observations.push(observation(entry.id, 'mode', 'single', base - 0.05, 4));
  }
...
check('parseChartSpec accepts a minimal spec', () => {
...
check('parseChartSpec rejects unknown type, level, sort and control', () => {
...
check('parseChartSpec allows controls: false', () => {
...
check('a valid spec validates clean against the real dataset', () => {
...
check('validation names the unknown metric and lists what exists', () => {
...
check('validation rejects an unknown level, facet, filter and id', () => {
  assert.match(validateChartSpec(real, { data: DATASET_ID, metric: 'coverage', level: 'raw' })[0], /level/);
  assert.match(
    validateChartSpec(real, { data: DATASET_ID, metric: 'coverage', level: 'breakdown', facet: 'nope' })[0],
    /unknown facet/,
  );
  assert.match(
  ...
  assert.match(validateChartSpec(real, { data: DATASET_ID, metric: 'coverage', entities: ['ghost'] })[0], /unknown entities/);
  assert.match(validateChartSpec(real, { data: DATASET_ID, metric: 'coverage', highlight: ['ghost'] })[0], /unknown highlight/);
  assert.match(validateChartSpec(real, { data: DATASET_ID, metric: 'coverage', colorBy: 'mode' })[0], /colorBy/);
});

check('validation catches a spec that names a different dataset than the file', () => {
  const problems = validateChartSpec(real, { data: 'other/set', metric: 'coverage' });
  assert.match(problems[0], /asks for dataset "other\/set"/);
});

check('validation refuses a chart-kind file', () => {
...
check('facet scope is enforced per level', () => {
  ...
  assert.match(
    validateChartSpec(tiny, { data: 'test/tiny', metric: 'acc', level: 'breakdown', facet: 'experiment' })[0],
    /facets on a slice dimension/,
  );
  assert.match(
    validateChartSpec(tiny, { data: 'test/tiny', metric: 'acc', level: 'experiments', facet: 'mode' })[0],
    /facets on a entity dimension/,
  );
  assert.deepEqual(
...
check('every row is an observation from the file, field for field', () => {
  const specs = [
...
check('no row is dropped or duplicated against the published observations', () => {
  const spec = { data: DATASET_ID, metric: 'all_record_accuracy', level: 'runs' };
  const model = computeChartModel(real, spec);
...
check('the chart row shape is fixed — no item-level or gold-label field can appear', () => {
  const model = computeChartModel(real, { data: DATASET_ID, metric: 'coverage', level: 'summary' });
  ...
  for (const row of model.rows) {
    ...
    assert.ok(!/gold|expected_answer|item_?id|question_?id|label_?text/i.test(serialised));
  }
});

check('summary shows the headline arms, runs shows every arm', () => {
  const summary = computeChartModel(real, { data: DATASET_ID, metric: 'coverage', level: 'summary' });
...
check('a runs row carries the prediction files behind it, and nothing more', () => {
  const model = computeChartModel(real, { data: DATASET_ID, metric: 'coverage', level: 'runs' });
  const withFiles = model.rows.filter((row) => row.runs.length > 0);
  assert.equal(withFiles.length, model.rows.length, 'every arm should have at least one run file');
  for (const row of model.rows) {
...
check('filters narrow the arm set and never change a value', () => {
  ...
  for (const row of filtered.rows) {
    ...
    assert.deepEqual([row.value, row.ciLow, row.ciHigh, row.n], [before.value, before.ciLow, before.ciHigh, before.n]);
  }
});

check('a filter that matches nothing is empty, not zero', () => {
...
check('highlight changes nothing about the data', () => {
...
check('a missing slice is counted as missing, never filled in', () => {
  const model = computeChartModel(tiny, {
    data: 'test/tiny',
    metric: 'acc',
    level: 'breakdown',
  ...
  // `d` publishes no `single` observation.
...
check('breakdown facets share one x-domain', () => {
  const model = computeChartModel(tiny, {
    data: 'test/tiny',
    metric: 'acc',
    level: 'breakdown',
  ...
  assert.deepEqual(model.facets.map((facet) => facet.key), ['pairwise', 'single']);
});

check('experiments groups arms by an entity dimension, reading overall values', () => {
  const model = computeChartModel(tiny, {
    data: 'test/tiny',
    metric: 'acc',
    level: 'experiments',
  ...
  assert.deepEqual(model.facets.map((facet) => facet.key), ['E1', 'E2']);
  assert.deepEqual(model.facets[0].rows.map((row) => row.entityId).sort(), ['a', 'b']);
  assert.deepEqual(model.facets[1].rows.map((row) => row.entityId).sort(), ['c', 'd']);
  for (const row of model.rows) {
    assert.equal(row.slice, 'overall');
    const source = tiny.observations.find(
      (entry) => entry.entity === row.entityId && entry.slice === 'overall' && entry.metric === 'acc',
    );
    assert.equal(row.value, source.value);
  }
});

check('switching level re-picks a facet of the right scope', () => {
...
check('sorting follows the measure direction and sinks missing values', () => {
  ...
  const lower = computeChartModel(tiny, { data: 'test/tiny', metric: 'latency', level: 'runs' });
  assert.equal(lower.rows[0].entityId, 'a');
...
check('a fraction measure never draws an axis outside 0–100%', () => {
  const model = computeChartModel(real, { data: DATASET_ID, metric: 'coverage', level: 'runs' });
  assert.ok(model.domain[0] >= 0, `domain starts at ${model.domain[0]}`);
  assert.ok(model.domain[1] <= 1, `domain ends at ${model.domain[1]}`);
});

check('table level is one row per published observation, and no finer', () => {
  ...
  const pairs = new Set(model.tableRows.map((row) => `${row.entityId}|${row.slice}|${row.sliceValue}`));
  assert.equal(pairs.size, model.tableRows.length, 'table rows must be unique per arm and slice');
});

check('an unknown metric falls back to the first measure instead of crashing', () => {
...
check('the layout stacks below 560px and never rotates a label', () => {
...
check('height grows with the row count', () => {
...
check('the static SVG is literal colour, one file per theme', () => {
  ...
  for (const result of [light, dark]) {
    assert.ok(result.svg.startsWith('<?xml version="1.0" encoding="UTF-8"?>'), 'needs an XML prologue');
...
check('the static SVG carries the same rows as the interactive model', () => {
  ...
  assert.ok(result.svg.includes(String(model.rows.length)) || model.rows.length > 0);
});

check('the narrow render is a different file from the wide one', () => {
...
check('a fraction axis is clamped, so no negative percentage is drawn', () => {
  const spec = { data: DATASET_ID, metric: 'coverage', level: 'summary' };
  const result = renderResearchChartSvg(real, spec, { theme: 'light', width: CHART_WIDTHS.wide });
  assert.ok(!/>\s*-\d/.test(result.svg), 'axis ticks must not go negative for a fraction measure');
});

check('table level renders through the table renderer', () => {
  const spec = { data: DATASET_ID, metric: 'coverage', level: 'table' };
  const result = renderResearchChartSvg(real, spec, { theme: 'light', width: CHART_WIDTHS.wide });
  assert.ok(result.svg.includes('Arm'), 'table renderer draws its own header');
...
check('the CSV is a re-export of the published rows', () => {
  ...
  for (const line of lines.slice(1)) assert.equal(line.split(',').length >= 12, true);
});

check('CSV cells are quoted per RFC 4180', () => {
  const csv = buildCsv({
    datasetId: 'test/tiny',
    metric: 'acc',
...
check('the download file name is safe and stable', () => {
  assert.equal(
...
check('findChartFences reads a body and stops at the closing fence', () => {
...
check('findChartFences handles CRLF, indentation and non-chart fences', () => {
  const markdown = [
    '```js',
    'const x = 1;',
    '```',
...
check('the demo chart in db-r-2026-010.md parses and validates', async () => {
  const markdown = await readFile(DEMO_PAPER, 'utf8');
  const fences = findChartFences(markdown, DEMO_PAPER).filter((fence) => fence.paperId === undefined);
  ...
  const problems = validateChartSpec(real, parsed.spec);
  assert.deepEqual(problems, []);
});

check('the demo chart did not disturb the rest of the paper', async () => {
  const markdown = await readFile(DEMO_PAPER, 'utf8');
  const lines = markdown.replace(/\r\n/g, '\n').split('\n');
  const open = lines.findIndex((line) => line.trim() === '```chart');
  assert.ok(open > 0, 'expected the fence to be on its own line');
  const close = lines.findIndex((line, index) => index > open && line.trim() === '```');
  assert.ok(close > open);
  assert.equal(close - open, 11, 'the demo fence should stay the size it was reviewed at');
  assert.equal(lines.filter((line) => line.trim() === '```chart').length, 1);
});

check('the slug is a pure function of the spec', () => {
...
if (!existsSync(manifestPath)) {
  ...
  check('every fence has a rendered chart in the manifest', () => {
  ...
  check('the manifest records provenance for every chart', () => {
    for (const chart of manifest.charts) {
      ...
      for (const variant of Object.values(chart.variants)) {
        ...
        assert.ok(existsSync(path.join(ROOT, 'frontend', 'public', variant.path.replace(/^\//, ''))));
      }
    }
  });

  check('the manifest covers both themes at both widths', () => {
    for (const chart of manifest.charts) {
      assert.deepEqual(
...
check('globals.css declares exactly the tokens theme.ts builds', () => {
...
check('every --rc-* variable the chart asks for is declared', async () => {
  const css = await readFile(GLOBALS_CSS, 'utf8');
