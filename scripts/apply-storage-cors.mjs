...
const corsFile = resolve(root, 'additionals/archive/firebase/storage.cors.json');
...
if (!ok) {
  console.error(
    '[storage:cors] Failed. Install Google Cloud SDK, run: gcloud auth login\n' +
      `Then: gcloud storage buckets update ${gsUrl} --cors-file=additionals/archive/firebase/storage.cors.json`,
  );
  process.exit(1);
}
