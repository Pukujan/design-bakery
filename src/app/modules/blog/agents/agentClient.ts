...
function extractCallableMessage(error: FirebaseError): string {
  ...
  if (error.code === 'functions/not-found' || /404|not found/i.test(raw)) {
    return formatCallableHttpError(raw || 'Not found', 404);
  }

  if (error.code === 'functions/unavailable' || error.code === 'functions/deadline-exceeded') {
    return `${raw || error.code} — Is the Functions emulator running? Restart: pnpm run dev`;
  }

  if (error.code === 'functions/internal') {
    return `${raw || 'Internal server error'} — Check the [fn] lines in your pnpm run dev terminal for the real error.`;
  }
...
export async function invokeBlogAgent(params: InvokeParams): Promise<AgentInvokeResponse> {
  let fnInstance;
  try {
    fnInstance = getFirebaseFunctions();
  } catch {
    return {
      ok: false,
      code: 'INTERNAL',
